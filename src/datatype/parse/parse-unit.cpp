...
_bool parseOneToken(_uro& uro, const Tokens& tks, _genptr<_bool>& result)
{
   ...
   switch (tk.type) {
      ...
      case Token::t_Word: {
         return uro.contextes.getVariable(tk, result, uro);
      }
      default: {
         return false;
      }
...
_bool parseOneToken(_uro& uro, const Tokens& tks, _genptr<_num>& result)
{
   ...
   switch (tk.type) {
      ...
      case Token::t_TwoWords: {
         ...
         _genptr<_tim> var;
         if (!uro.contextes.getVariable(tk, var, uro)) {
...
{
   const Token& tk = tks.first();
   return tk.type == Token::t_Word && uro.contextes.getVariable(tk, result, uro);
};
